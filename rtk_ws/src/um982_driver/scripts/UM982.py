...
class UM982Serial(threading.Thread):
    def __init__(self, port, band):
        ...
        self.vel            = None   # 速度
        self.utmpos         = None
        # 读取初始数据，直到获得有效的位置信息或达到最大尝试次数
        max_attempts = 100
        attempts = 0
        
        print("Starting to read from UM982...")
        while self.fix is None and attempts < max_attempts:
            self.read_frame()
            attempts += 1
            time.sleep(0.01)  # 防止CPU过度占用
        
        if self.fix is None:
            print("ERROR: Failed to get PVTSLNA message after %d attempts" % max_attempts)
            self.stop()
            return
        # wgs84转utm
    ...
    def read_frame(self):
        ...
        if frame.startswith("#PVTSLNA") and nmea_expend_crc(frame):
            ...
            # print(f"Got BESTNAVA: Vx={self.vel[0]:.3f} m/s, Vy={self.vel[1]:.3f} m/s, Vz={self.vel[2]:.3f} m/s")
    ...
    def run(self):
        while self.isRUN:
            ...
            bestpos_hgt, bestpos_lat, bestpos_lon, bestpos_hgtstd, bestpos_latstd, bestpos_lonstd = self.fix
            self.utmpos = utm_trans(self.transformer, bestpos_lon, bestpos_lat)
            # print("self.utmpos",self.utmpos)




if __name__ == "__main__":
    um982 = UM982Serial("/dev/ttyUSB0", 921600)
    um982.start()
